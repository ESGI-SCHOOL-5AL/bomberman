...
class Policy:  # MLPRegressor
    ...
    def choose_action(self, state):
        ...
        self.epsilon = max(self.epsilon - self.epsilon_step,
                           self.final_epsilon)
        return action
    ...
    def update(self, previous_state, state, last_action, reward):
        ...
        if batch_result:
            self.mlp.fit(np.array(batch_result[0]).reshape(-1, len(inputs[0])), np.array(
                batch_result[1]).reshape(-1, len(outputs[0])))
            self.batch.reset()
